...
export default function ServicesPage() {
    ...
    useEffect(() => { fetch('/api/data?entity=services').then(r => r.json()).then(d => { setItems(d.data || []); setLoading(false); }); }, []);
    ...
    const handleSave = async () => {
        ...
        if (editItem) {
            await fetch('/api/data', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ entity: 'services', action: 'update', data: { id: editItem._id, updates: form } }) });
            setItems(prev => prev.map(s => s._id === editItem._id ? { ...s, ...form } : s));
            ...
            const d = await res.json(); setItems(prev => [...prev, d.data]); addToast('success', 'Layanan ditambahkan');
        }
        setShowModal(false);
    };
    ...
    return (
        <div>
            <div className="page-header"><div className="page-header-left"><h1 className="page-title">Layanan</h1><p className="page-subtitle">Jenis layanan pengiriman</p></div>
                <div className="page-actions"><button className="btn btn-primary" onClick={openNew}><Plus size={18} /> Tambah Layanan</button></div></div>
                        ...
                        items.length === 0 ? <tr><td colSpan={4}><div className="empty-state"><Layers size={48} className="empty-state-icon" /><div className="empty-state-title">Belum ada layanan</div></div></td></tr> :
                            items.map(s => (
                                <tr key={s._id}><td className="font-semibold">{s.name}</td><td className="text-muted">{s.description}</td>
                                    ...
                                    <td><button className="table-action-btn" onClick={() => openEdit(s)}><Edit size={14} /> Edit</button></td></tr>
                            ))}
                </tbody>
            </table></div></div>
            {showModal && (
